...
![预留 IPv6 地址的截图](/images/2016/gce-anycast-1.png)
...
![为 Anycast 建立的两个实例](/images/2016/gce-anycast-2.png)
...
![实例组配置页面](/images/2016/gce-anycast-3.png)
...
![三种负载均衡模式](/images/2016/gce-anycast-4.png)

在负载均衡的配置界面，把这两个实例组都添加到 “后端” 中。 该功能还需要创建一个运行状态检查（相当于监控功能），当主机宕机后能实现切换。

![暂时先不开启 CDN 功能](/images/2016/gce-anycast-5.png)

![保留默认的 “主机路径和规则”](/images/2016/gce-anycast-6.png)

![目前是需要 HTTP 的例子，如果需要 HTTPS，还需要指定一个证书。](/images/2016/gce-anycast-7.png)

创建成功后，可以看到如下界面，其中的 IP 地址就是 Anycast IP 了。 \[img id="2009" size="large"\]\[/img\]

![成功创建了一个 Anycast IP](/images/2016/gce-anycast-8.png)
...
![对 Anycast IP 的国外速度测试](/images/2016/gce-anycast-9.png)

中国的速度那更是一流的快，Google 有香港的边缘节点，所以基本上是直接走的香港节点，比原本的连接台湾可用区快不少。（只有部分 IP 段是完全直连的）

![对 Anycast IP 的国内速度测试](/images/2016/gce-anycast-10.png)

### HTTP GET 测试

在开启 CDN 功能之前，负载均衡器是不会对任何内容缓存的，所以会发现 Connect 的速度很快，但是 TTFB 延迟还是有不少。

![对 Anycast IP 进行 HTTP GET 测试](/images/2016/gce-anycast-11.png)
...
![开启 CDN 后进行 HTTP GET 测试](/images/2016/gce-anycast-12.png)
...
![缓存服务器的位置](/images/2016/gce-anycast-13.png)
...
![图形还是很漂亮的](/images/2016/gce-anycast-14.png)

### 延迟日志

在网页后台的 Stackdriver，Trace 下，可以看到延迟日志： 

![延迟日志截图 1](/images/2016/gce-anycast-15.png)

![延迟日志截图 2](/images/2016/gce-anycast-16.png)
