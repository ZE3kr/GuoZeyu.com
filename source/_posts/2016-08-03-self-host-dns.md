...
![添加子域名服务器记录](/images/2016/powerdns-1.png)
...
可以看到，在这个 org 的 NS 服务器就已经把 ns.icann.org. 的记录返回来了，这也就是你需要在域名注册商填写 IP 地址的原因。然而你最好在你域名下的 DNS 服务器上也返回相同的 NS 和相同的 IP。 最后，不要忘了改域名的 NS 记录： \[img id="1838" size="large"\]\[/img\]
